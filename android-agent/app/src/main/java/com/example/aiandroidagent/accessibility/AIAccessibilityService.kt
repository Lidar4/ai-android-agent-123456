...
class AIAccessibilityService : AccessibilityService() {
    ...
    companion object {
        ...
        fun submitCommand(command: String): Boolean =
            command.isNotBlank() && _commands.tryEmit(command.trim())
        ...
        fun swipe(x1: Float, y1: Float, x2: Float, y2: Float, duration: Long): Boolean {
            ...
            return service.dispatchGesture(
                GestureDescription.Builder().addStroke(GestureDescription.StrokeDescription(path, 0, safeDuration)).build(),
                null,
                null
            )
        }
    ...
    override fun onServiceConnected() {
        ...
        instance = this
        orchestrator = AgentOrchestrator(GeminiProvider(), ActionPolicy()) { action ->
            DeviceActionExecutor(this).execute(action)
        }
        serviceScope.launch {
    ...
    private fun runCommand(command: String) {
        ...
        agentJob = serviceScope.launch {
            orchestrator.run(
                command = command,
                state = { DeviceState(accessibilityNodes = _snapshot.value) },
    ...
    private fun findText(text: String): AccessibilityNodeInfo? =
        rootInActiveWindow?.let { root -> findFirst(root) { it.text?.toString()?.equals(text, ignoreCase = true) == true } }

    private fun findDescription(text: String): AccessibilityNodeInfo? =
        rootInActiveWindow?.let { root -> findFirst(root) { it.contentDescription?.toString()?.equals(text, ignoreCase = true) == true } }
